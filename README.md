...
В процессе также разбирался классический перцептрон и MLP (multi-layer perceptron) с нуля на numpy/sklearn — на CIFAR-10, с классами кошка/собака/птица. Тот код в проде не используется (заменён на предобученную модель для лучшей точности), но остаётся как учебный пример.
